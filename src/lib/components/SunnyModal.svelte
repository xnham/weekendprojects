<script lang="ts">
  ...
  // Updated interface - we'll use Record to define the return type of off()
  interface VapiInstance {
    on: (event: string, callback: Function) => void;
    off: (eventName: string | symbol, listener: (...args: any[]) => void) => Record<string, any>;
    stop: () => void;
    start: (assistantId: string | object, assistantOverrides?: object) => Promise<any>;
    send: (message: { type: string; message: { role: string; content: string } }) => void;
    isMuted: () => boolean;
    setMuted: (muted: boolean) => void;
    say: (message: string, endCallAfterSpoken?: boolean) => void;
    getDailyCallObject: () => any;
    [key: string]: any;
  }

  // Define proper type for volume and other parameters
  type VolumeLevel = number;
  type MessageEvent = { text?: string; type?: string; [key: string]: any };
  type ErrorEvent = { message: string; [key: string]: any };

  // Properly define the assistantId
  const assistantId = "24c467e9-e11f-4ad1-8643-ddee12f1ab17"; // Use the actual assistant ID
  ...
  onMount(() => {
    if (browser) {
      ...
      import('@vapi-ai/web').then(module => {
        ...
        vapi = new Vapi("18aebfed-8669-4ace-beec-e219652fc662") as unknown as VapiInstance;
    ...
    try {
      ...
      try {
        ...
        if (dailyCallObject) {
          ...
          setTimeout(() => {
            try {
              if (dailyCallObject && dailyCallObject.participants && dailyCallObject.participants()) {
                console.log(
                  "Daily participants:",
                ...
                dailyCallObject.on && dailyCallObject.on("track-started", (event: any) => {
      ...
      vapi.on("volume-level", (volume: VolumeLevel) => {
        volumeLevel = volume;
        lastVolumeTime = Date.now();
      ...
      vapi.on("message", (message: MessageEvent) => {
      ...
      vapi.on("error", (error: ErrorEvent) => {
