...
const _require$1 = (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('main.cjs', document.baseURI).href)) ? module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('main.cjs', document.baseURI).href))) : require;
...
function map_layout_to_path(
	...
	const match = Object.keys(layout_map).find((l) =>
		new RegExp(`\\` + `${path__namespace.sep}${l}` + `\\` + `${path__namespace.sep}`).test(
			path__namespace.normalize(filename).replace(process.cwd(), '')
		)
	);
...
const _require = (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('main.cjs', document.baseURI).href)) ? module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('main.cjs', document.baseURI).href))) : require;
...
function resolve_layout(layout_path) {
	try {
		...
		try {
			const _path = path$2.join(process.cwd(), layout_path);
			return to_posix(_require.resolve(_path));
		} catch (e) {
			throw new Error(
				`The layout path you provided couldn't be found at either ${layout_path} or ${path$2.join(
					process.cwd(),
					layout_path
...
exports.code_highlighter = code_highlight;
